...
import { loginValidet } from "../utlis/CheckValidet";
...
const LoginPage = () => {
    ...
    useEffect(() => {
        if (Object.keys(errors).length === 0 && isSubmit) {
            ...
            } else if (users[userIndex].password !== values.password) {
                setError({ password: "Invalid email or password" });
            } else {
                localStorage.setItem("currentLogin", JSON.stringify(users[userIndex]));
                navigate("/");
            }
        }
    }, [errors, isSubmit]);
...
return (
  ...
  <div className="container px-5 py-24 vovol ">
    <Scroll/>
  </div>
      ...
      <form className="mx-auto max-w-lg rounded-lg border" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-4 p-4 md:p-8">
          ...
          <div>
            ...
            {errors.email && <p className="hspc">{errors.email}</p>}
          </div>
          ...
          <div>
            ...
            {errors.password && <p className="hspc">{errors.password}</p>}
          </div>
  
          <button className="block rounded-lg bg-gray-800 px-8 py-3 text-center text-sm font-semibold text-white outline-none ring-gray-300 transition duration-100 hover:bg-gray-700 focus-visible:ring active:bg-gray-600 md:text-base">Login</button>
        ...
        <div className="flex items-center justify-center bg-gray-100 p-4">
          <p className="text-center text-sm text-gray-500">Don't Have an Account? <Link to='/signup' class="text-indigo-500 transition duration-100 hover:text-indigo-600 active:text-indigo-700">Sign up now</Link></p>
        </div>
      </form>
    </div>
  </div> 
